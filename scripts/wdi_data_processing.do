...
drop if _n>=652 // delete footnotes in database
...
* Macro for period selection (overall horizon: 1996-2021)
local min_year = "2015"
local max_year = "2021"
...
keep country* yr`min_year'-yr`max_year' // keep years within selected period

* Convert string into numerical data
forvalues t = `min_year'/`max_year'{
	replace yr`t' = "" if yr`t'==".." // define missing data
}
destring yr*, replace
