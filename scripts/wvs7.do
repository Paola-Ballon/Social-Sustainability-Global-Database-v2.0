...
Issues:
	- The following countries do not have information on ethnic groups: [subgroup]
		* Germany, Japan, Jordan, Lebanon, New Zealand, Turkey
...
levelsof S003, local(countries)
foreach x of local countries{
	if (`x'!=792 & `x'!=554 & `x'!=422 & `x'!=400 & `x'!=392 & `x'!=276){ // Recall that some countries do not have information on ethnic groups
...
* corrections
drop if countryname=="Northern Ireland" // since we are not sure this is a country itself
...
save "$proc_data\wvs7.dta", replace
